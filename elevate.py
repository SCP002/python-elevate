...
def elevate_to_system():
    ...
    try:
        winlogon_pid = _get_process_pid("winlogon.exe")
...
def elevate_to_trusted_installer():
    """
...
def _is_user_sid_equals(target_sid: Any) -> bool:
    """
...
def _get_service_process_pid(service_name: str) -> int:
    ...
    try:
        ...
        try:
            status: dict = win32service.QueryServiceStatusEx(service_handle)  # type: ignore
...
def _get_current_token_groups() -> list[tuple[Any, int]] | None:
    """
    ...
    try:
        ...
        return win32security.GetTokenInformation(token_handle, win32security.TokenGroups)
    except Exception as e:
        print(f"Error retrieving token groups: {e}", file=sys.stderr)
        return None


def _is_sid_enabled_in_groups(groups: list[tuple[Any, int]], sid_to_check: Any) -> bool:
    """
...
def _get_process_pid(process_name: str) -> int:
    """Returns PID of the process with given name"""
    for proc in psutil.process_iter(["pid", "name"]):
        if proc.info["name"].lower() == process_name:
            return proc.info["pid"]
    else:
        raise psutil.NoSuchProcess(f"Unable to find {process_name} process")
